...
export function applyTheme(flavor: CatppuccinFlavor) {
  const theme = CATPPUCCIN_THEMES[flavor];
...
export function getStoredFlavor(): CatppuccinFlavor {
  const stored = localStorage.getItem("catppuccin-flavor");
  return (stored as CatppuccinFlavor) || "mocha";
}
